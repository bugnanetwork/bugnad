...
var serviceDescription = &winservice.ServiceDescription{
	Name:        "bugnadsvc",
	DisplayName: "Bugnad Service",
	Description: "Downloads and stays synchronized with the Bugna blockDAG and " +
		"provides DAG services to applications.",
}

type bugnadApp struct {
	cfg *config.Config
}

// StartApp starts the bugnad app, and blocks until it finishes running
func StartApp() error {
...
func (app *bugnadApp) main(startedChan chan<- struct{}) error {
	...
	if err != nil {
		log.Errorf("Unable to start bugnad: %+v", err)
		return err
	}

	defer func() {
		log.Infof("Gracefully shutting down bugnad...")
		...
		log.Infof("Bugnad shutdown complete")
	}()
