...
// Outpoint defines a bugna data type that is used to track previous
// transaction outputs.
...
// NewOutpoint returns a new bugna transaction outpoint point with the
...
// TxIn defines a bugna transaction input.
type TxIn struct {
...
// NewTxIn returns a new bugna transaction input with the provided
...
// TxOut defines a bugna transaction output.
type TxOut struct {
	Value        uint64
	ScriptPubKey *externalapi.ScriptPublicKey
}

// NewTxOut returns a new bugna transaction output with the provided
...
// MsgTx implements the Message interface and represents a bugna tx message.
