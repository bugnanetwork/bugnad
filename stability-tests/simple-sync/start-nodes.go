...
func startNodes() (teardown func(), err error) {
	...
	syncerDataDir, err := common.TempDir("bugnad-datadir-syncer")
	if err != nil {
		panic(errors.Wrapf(err, "error in Tempdir"))
	}
	log.Infof("SYNCER datadir: %s", syncerDataDir)

	syncedDataDir, err := common.TempDir("bugnad-datadir-synced")
	if err != nil {
		panic(errors.Wrapf(err, "error in Tempdir"))
	}
	log.Infof("SYNCED datadir: %s", syncedDataDir)

	syncerCmd, err := common.StartCmd("BGAPAD-SYNCER",
		"bugnad",
	...
	syncedCmd, err := common.StartCmd("BGAPAD-SYNCED",
		"bugnad",
		common.NetworkCliArgumentFromNetParams(activeConfig().NetParams()),
