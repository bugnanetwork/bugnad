...
func TestVirtualSelectedParentChain(t *testing.T) {
	// Setup a couple of bugnad instances
	bugnad1, bugnad2, _, teardown := standardSetup(t)
	defer teardown()
	...
	err := bugnad1.rpcClient.RegisterForVirtualSelectedParentChainChangedNotifications(true,
	...
	// In bugnad2, mine a different chain of `blockAmountToMine` + 1
	// blocks over the genesis
	var chain2Tip *externalapi.DomainBlock
	for i := 0; i < blockAmountToMine+1; i++ {
		chain2Tip = mineNextBlock(t, bugnad2)
	}

	// Connect the two bugnads. This should trigger sync
	// between the two nodes
	connect(t, bugnad1, bugnad2)
	...
	// the first chain
	virtualSelectedParentChainFromChain1Tip, err := bugnad1.rpcClient.GetVirtualSelectedParentChainFromBlock(
