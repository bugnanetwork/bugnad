...
func TestCreateDefaultConfigFile(t *testing.T) {
	...
	sampleConfigFile := filepath.Join(filepath.Dir(path), "sample-bugnad.conf")

	// Setup a temporary directory
	tmpDir, err := ioutil.TempDir("", "bugnad")
	if err != nil {
		t.Fatalf("Failed creating a temporary directory: %v", err)
	}
	testpath := filepath.Join(tmpDir, "test.conf")

	// copy config file to location of bugnad binary
	...
	tmpConfigFile := filepath.Join(appPath, "sample-bugnad.conf")
