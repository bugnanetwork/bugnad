...
Clients are expected to build RequestMessages and wrap them in BugnadMessage. (see messages.proto)

Having received a RequestMessage, (wrapped in a BugnadMessage) the RPC server will respond with a
ResponseMessage (likewise wrapped in a BugnadMessage) respective to the original RequestMessage.
...
GetCurrentNetworkRequestMessage requests the network bugnad is currently running against.
...
| Field | Type | Label | Description |
| ----- | ---- | ----- | ----------- |
| payAddress | [string](#string) |  | Which bugna address should the coinbase block reward transaction pay into |
...
| ----- | ---- | ----- | ----------- |
| block | [RpcBlock](#protowire.RpcBlock) |  |  |
| isSynced | [bool](#bool) |  | Whether bugnad thinks that it&#39;s synced. Callers are discouraged (but not forbidden) from solving blocks when bugnad is not synced. That is because when bugnad isn&#39;t in sync with the rest of the network there&#39;s a high chance the block will never be accepted, thus the solving effort would have been wasted. |
| error | [RPCError](#protowire.RPCError) |  |  |
...
GetPeerAddressesRequestMessage requests the list of known bugnad addresses in the
...
currently connected to this bugnad.
...
| isOutbound | [bool](#bool) |  | Whether this bugnad initiated the connection |
| timeOffset | [int64](#int64) |  |  |
| userAgent | [string](#string) |  |  |
| advertisedProtocolVersion | [uint32](#uint32) |  | The protocol version that this peer claims to support |
| timeConnected | [int64](#int64) |  | The timestamp of when this peer connected to this bugnad |
...
AddPeerRequestMessage adds a peer to bugnad&#39;s outgoing connection list.
This will, in most cases, result in bugnad connecting to said peer.
...
parent chain from some startHash to this bugnad&#39;s current virtual
...
GetBlockCountRequestMessage requests the current number of blocks in this bugnad.
...
ShutDownRequestMessage shuts down this bugnad.
...
This call is only available when this bugnad was started with `--utxoindex`
...
GetUtxosByAddressesRequestMessage requests all current UTXOs for the given bugnad addresses
