...
func main() {
	...
	if !cfg.AllowConnectionToDifferentVersions {
		bugnadMessage, err := client.Post(&protowire.BugnadMessage{Payload: &protowire.BugnadMessage_GetInfoRequest{GetInfoRequest: &protowire.GetInfoRequestMessage{}}})
		if err != nil {
			printErrorAndExit(fmt.Sprintf("Cannot post GetInfo message: %s", err))
		}

		localVersion := version.Version()
		remoteVersion := bugnadMessage.GetGetInfoResponse().ServerVersion
...
func prettifyResponse(response string) string {
	bugnadMessage := &protowire.BugnadMessage{}
	err := protojson.Unmarshal([]byte(response), bugnadMessage)
	if err != nil {
	...
	return marshalOptions.Format(bugnadMessage)
}
