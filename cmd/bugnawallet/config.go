...
type createConfig struct {
	KeysFile          string `long:"keys-file" short:"f" description:"Keys file location (default: ~/.bugnawallet/keys.json (*nix), %USERPROFILE%\\AppData\\Local\\Bugnawallet\\key.json (Windows))"`
...
type sendConfig struct {
	...
	Password                 string   `long:"password" short:"p" description:"Wallet password"`
	DaemonAddress            string   `long:"daemonaddress" short:"d" description:"Wallet daemon server to connect to"`
	ToAddress                string   `long:"to-address" short:"t" description:"The public address to send Bugna to" required:"true"`
	FromAddresses            []string `long:"from-address" short:"a" description:"Specific public address to send Bugna from. Use multiple times to accept several addresses" required:"false"`
	SendAmount               string   `long:"send-amount" short:"v" description:"An amount to send in Bugna (e.g. 1234.12345678)"`
	IsSendAll                bool     `long:"send-all" description:"Send all the Bugna in the wallet (mutually exclusive with --send-amount)"`
...
func parseCommandLine() (subCommand string, config interface{}) {
	...
	parser.AddCommand(balanceSubCmd, "Shows the balance of a public address",
		"Shows the balance for a public address in Bugna", balanceConf)

	sendConf := &sendConfig{DaemonAddress: defaultListen}
	parser.AddCommand(sendSubCmd, "Sends a Bugna transaction to a public address",
		"Sends a Bugna transaction to a public address", sendConf)
	...
	parser.AddCommand(createUnsignedTransactionSubCmd, "Create an unsigned Bugna transaction",
		"Create an unsigned Bugna transaction", createUnsignedTransactionConf)
