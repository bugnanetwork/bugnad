...
func (s *server) mergeTransaction(
	...
	utxos := make([]*libbugnawallet.UTXO, len(splitTransactions))
	for i, splitTransaction := range splitTransactions {
		output := splitTransaction.Tx.Outputs[0]
		utxos[i] = &libbugnawallet.UTXO{
	...
	payments := []*libbugnawallet.Payment{{
		Address: toAddress,
		Amount:  sentValue,
	}}
	if totalValue > sentValue {
		payments = append(payments, &libbugnawallet.Payment{
			Address: changeAddress,
			Amount:  totalValue - sentValue,
		})
	}

	mergeTransactionBytes, err := libbugnawallet.CreateUnsignedTransaction(s.keysFile.ExtendedPublicKeys,
...
func (s *server) splitAndInputPerSplitCounts(transaction *serialization.PartiallySignedTransaction, transactionMass uint64,
	...
	// Since the transaction was generated by bugnawallet, we assume all inputs have the same number of signatures, and
	// thus - the same mass.
...
func (s *server) createSplitTransaction(transaction *serialization.PartiallySignedTransaction,
	...
	selectedUTXOs := make([]*libbugnawallet.UTXO, 0, endIndex-startIndex)
	totalSompi := uint64(0)
	...
	for i := startIndex; i < endIndex && i < len(transaction.PartiallySignedInputs); i++ {
		...
		selectedUTXOs = append(selectedUTXOs, &libbugnawallet.UTXO{
	...
	unsignedTransactionBytes, err := libbugnawallet.CreateUnsignedTransaction(s.keysFile.ExtendedPublicKeys,
...
func (s *server) estimateMassAfterSignatures(transaction *serialization.PartiallySignedTransaction) (uint64, error) {
	...
	transactionWithSignatures, err := libbugnawallet.ExtractTransactionDeserialized(transaction, s.keysFile.ECDSA)
	if err != nil {
		return 0, err
	}
...
func (s *server) moreUTXOsForMergeTransaction(alreadySelectedUTXOs []*libbugnawallet.UTXO, requiredAmount uint64) (
	additionalUTXOs []*libbugnawallet.UTXO, totalValueAdded uint64, err error) {
	...
	for _, utxo := range s.utxosSortedByAmount {
		...
		additionalUTXOs = append(additionalUTXOs, &libbugnawallet.UTXO{
